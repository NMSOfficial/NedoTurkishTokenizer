...
\paragraph{Lossless roundtrip validation.}
A separate lossless roundtrip check was run on 10,000 cases (10000 cases).
The check produced 0 mismatches.
This result applies to \texttt{tokenize\_lossless()} followed by \texttt{detokenize()}, and should not be interpreted as a guarantee for every normal \texttt{tokenize()} call.

\section*{Limitations}

The reported results are limited to the evaluation sets described in this paper.
The main Final1000 set measures morphology-aware tokenization on curated Turkish examples.
Performance is weaker on code-mixed foreign-root forms with Turkish suffixes, and this portion of the benchmark should be expanded in future work.
The lossless roundtrip result applies to the explicit lossless API rather than to every normal tokenization call.

\section*{Data and Code Availability}

The source code of NedoTurkishTokenizer is available at:
\url{https://github.com/NMSOfficial/NedoTurkishTokenizer}.

The Final1000 evaluation dataset is available on Hugging Face:
