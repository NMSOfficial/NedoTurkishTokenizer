...
words=['edeceğim','edeceğiz','shirtlerimiz','slotlarından','mekanlarından','hemşerilerimiz','işbirliklerinden','birbirlerinden']
with open('reports/full_validation_v21/debug_remaining.txt','w',encoding='utf-8') as f:
 for w in words:
  cs=generate_candidates(w, load_tdk_words(), ALL_DOMAIN_ROOTS, frozenset()); b=select_best_candidate(cs)
  f.write('\nWORD '+w+' N '+str(len(cs))+'\n')
  for c in cs[:15]:
