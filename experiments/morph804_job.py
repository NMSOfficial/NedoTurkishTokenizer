...
try:
    ...
    gold=[json.loads(x) for x in pathlib.Path('reports/q1_final/final_gold_804.jsonl').read_text(encoding='utf-8').splitlines() if x.strip()]
    def bd(a):
        b=set(); n=0
...
outp.joinpath('morpheus_neural_804.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
