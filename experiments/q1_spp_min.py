...
for line in open(raw,encoding='utf-8',errors='ignore'):
 ...
 if txt.strip(): lines.append(txt.replace('\n',' '))
 if len(lines)>=20000: break
W=B/'spp_work'; W.mkdir(exist_ok=True); train=W/'train.txt'; train.write_text('\n'.join(lines),encoding='utf-8')
res={'ntrain':len(lines)}
...
(B/'q1_spp_baselines.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
