...
T=NedoTurkishTokenizer(); G=[json.loads(x) for x in pathlib.Path('reports/q1_worldclass_final/full_gold_1000.jsonl').read_text(encoding='utf-8').splitlines() if x.strip()]
def bd(a):
 b=set(); n=0
...
pathlib.Path('reports/q1_worldclass_final/morphological_probe_1000.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
